...
<body>
	<div id="wrap" class="container w-75 h-100">
		...
		<header
			...
			<!-- 로고영역 -->
			<h1 class="home-tag fw-bold text-center my-3" id="title">
		...
		<p class="text-center fw-bold fs-3 my-5">
			강사 매출 현황 <i class="fa-solid fa-arrow-trend-up"></i>
		</p>

		<table class="d-flex justify-content-center my-5 opacity-75">
			<tr class="border = 1 text-center bg-dark">
				<td class="border = 1 p-4 fw-bold px-5 text-light">강사 코드</td>
				<td class="border = 1 p-4 fw-bold px-5 text-light">강의명</td>
				<td class="border = 1 p-4 fw-bold px-5 text-light">강사명</td>
				<td class="border = 1 p-4 fw-bold px-5 text-light">총 매출</td>
			</tr>
			<c:forEach var="result" items="${resultList}">

				<tr class="border = 1 text-center">
					<td
						class="border border-dark border = 1 p-4 fw-bold text-dark px-5 bg-light">${result.teacher_code}</td>
					<td
						class="border border-dark border = 1 p-4 fw-bold text-dark px-5 bg-light">${result.class_name}</td>
					<td
						class="border border-dark border = 1 p-4 fw-bold text-dark px-5 bg-light">${result.teacher_name}</td>
					<td
						class="border border-dark border = 1 p-4 fw-bold text-dark px-5 bg-light">₩${result.total}</td>
				</tr>
			</c:forEach>
		</table>
